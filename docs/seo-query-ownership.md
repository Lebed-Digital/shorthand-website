...
- **Current cannibalization (CONFIRMED, query-level, 2026-07-17):** "preschool report card comments" (17 imp @23.3 behavior page vs 3 @40.3 general), "pre k report card comments" (14 @22.6 vs 6 @22.3), "3 year old preschool progress report comments" family split. Both stuck because Google can't pick.
- **Fix (proposal awaiting Greg's approval, drafted 2026-07-17):** differentiate, DO NOT merge — both pages earn clicks. General page gets age-based structure and keeps every generic query; behavior page leads with "Preschool Behavior" in title, never uses bare "preschool report card comments" in a heading, and receives the general page's self-regulation/routines comments. Full spec in the 2026-07-17 Agent Handoff entry.
- **Must not merge:** these two pages. Differentiate only.
- **Planned additions:** "Daily behavior report for preschool (free template)" (Aug 2026 post — will own that query, currently 9 imp landing wrong on the behavior page); "Kindergarten report card comments" (Oct 2026 post).
...
- **Primary:** `/blog/special-education-behavior-tracking-software` — **PARKED until 2026-08-13 audit** (prior decision; do not touch before then)
